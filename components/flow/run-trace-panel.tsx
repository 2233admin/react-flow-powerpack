...
export function RunTracePanel() {
  ...
  const runSimulation = async () => {
    ...
    try {
      ...
      const artifact = (await response.json()) as WorkflowRunArtifact | { message?: string }
      if (!response.ok || !("run" in artifact)) {
        throw new Error("message" in artifact && artifact.message ? artifact.message : "Workflow run failed")
      }
                  ...
                  <div className="space-y-2">
                    ...
                    {runState.artifact ? (
                      <p className="break-all font-mono text-[10px] leading-relaxed text-muted-foreground">
                        artifact: {runState.artifact.artifactPath}
                      </p>
                    ) : null}
                  </div>
                  <Separator />
